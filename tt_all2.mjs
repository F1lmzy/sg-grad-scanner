...
const res = await page.evaluate(async (E)=>{
  const out=[];
  // POST with NO body (matches site)
  const r=await fetch(E, {method:'POST'});
  const txt=await r.text();
  let j; try{ j=JSON.parse(txt); } catch(e){ return [{noBodyErr:String(e)}]; }
...
fs.writeFileSync('/tmp/tt_all2.json', JSON.stringify(res,null,1));
const meta=res.filter(x=>x.n!==undefined);
